...
var ball, paddle, score, playingArea;
var snd, music, sndEnabled = false, musicEnabled = false;
var aWidth, aHeight, pWidth, pHeight, dx = 2, dy = 2;
var pdx = 48; // The number of pixles the paddle will move on the screen
var currentScore = 0, timer, paddleLeft = 228, ballLeft = 100, ballTop = 8;
var drag = false;
var gear, controls, newButton, difficultySelect, doneButton;
var beepX, beepY, beepPaddle, beepGameOver, bgMusic;
...
function init(){
    ...
    // We want to minimize the calls to get element by ID
    // For that, we stored the DOM objects in local variables (Do this whenever possible!) 
...
function collisionY(){
    ...
    if(ballTop > pHeight - 64){
        if(ballLeft >= paddleLeft + 16 && ballLeft < paddleLeft + 48){
            if(dx < 0){
                dx = -2; // Keep the ball moving from right to left
            } else { dx = 2; } // The other direction - left to right
            playSound(beepPaddle);
            return true;
        } else if(ballLeft >= paddleLeft && ballLeft < paddleLeft + 16){
            if(dx < 0){ 
                dx = -8; // flatten out the trajectory of the ball, makeing it more difficult to track
            }else { dx = 8; }
            playSound(beepPaddle);
            return true;
